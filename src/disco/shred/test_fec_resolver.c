...
#define ADD_SHRED_THRASHES( resolver, shred, expected, expected_tslot, expected_tset, expected_tmax ) do {                                 \
  fd_shred_t const * __parsed = fd_shred_parse( (shred), 2048UL );                                                                         \
  ulong tslot = ULONG_MAX; uint  tset = FD_SHRED_BLK_MAX; uint tmax = FD_SHRED_BLK_MAX;                                                    \
  int retval = fd_fec_resolver_add_shred( resolver, __parsed, 2048UL, pubkey, out_fec, out_shred, out_merkle_root, &tslot, &tset, &tmax ); \
...
test_rolloff( void ) {
  ...
  for( ulong j=0UL; j<set2->data_shred_cnt; j++ ) {
    if( j == 0 ) { ADD_SHRED_THRASHES( resolver, set2->data_shreds[ j ], OKAY, 0UL, 0UL, 31UL ); }
  ...
  /* Now, set0 kicked out.  set1 and set2 are in progress.  if we add
     ...
     are in progress.*/
  ADD_SHRED_THRASHES( resolver, set0->parity_shreds[ 0 ], OKAY, 0UL, 32UL, 31UL );
...
test_new_formats( void ) {
  ...
  while( !fd_ar_read_next( file, meta ) ) {
    ...
    ulong thrashed_slot = ULONG_MAX; uint  thrashed_fec_set_idx = FD_SHRED_BLK_MAX; uint thrashed_max_dshred_idx = FD_SHRED_BLK_MAX;
    int retval = fd_fec_resolver_add_shred( resolver, parsed, shred_sz, pubkey, out_fec, out_shred, out_merkle_root, &thrashed_slot, &thrashed_fec_set_idx, &thrashed_max_dshred_idx );
    FD_TEST( thrashed_slot==ULONG_MAX );
    FD_TEST( thrashed_fec_set_idx==FD_SHRED_BLK_MAX );
    FD_TEST( thrashed_max_dshred_idx==FD_SHRED_BLK_MAX );
  ...
  for( ulong slot=10UL; slot<10UL+MAX_SLOTS; slot++ ) {
    ...
    for( ulong setid=0UL; setid<MAX_SETS; setid++ ) {
      ...
      for( ulong j=0; j<set->data_shred_cnt; j++ ) {
        ...
        int retval = fd_fec_resolver_add_shred( resolver, parsed, FD_SHRED_MIN_SZ, pubkey, out_fec, out_shred, out_merkle_root, &thrashed_slot, &thrashed_fec_set_idx, &thrashed_max_dshred_idx );
      ...
      for( ulong j=0; j<set->parity_shred_cnt; j++ ) {
        ...
        FD_TEST( parsed );
        ulong thrashed_slot = ULONG_MAX; uint  thrashed_fec_set_idx = FD_SHRED_BLK_MAX; uint thrashed_max_dshred_idx = FD_SHRED_BLK_MAX;
        int retval = fd_fec_resolver_add_shred( resolver, parsed, FD_SHRED_MAX_SZ, pubkey, out_fec, out_shred, out_merkle_root, &thrashed_slot, &thrashed_fec_set_idx, &thrashed_max_dshred_idx );
        FD_TEST( thrashed_slot==ULONG_MAX );
        FD_TEST( thrashed_fec_set_idx==FD_SHRED_BLK_MAX );
        FD_TEST( thrashed_max_dshred_idx==FD_SHRED_BLK_MAX );
